...
#         # Adi's State:
#         if reset:
#             return tuple([self.sim.roads[0].traffic_signal_state, 0, 0, 0])
#         # West, east inbound = [0, 2]
#         # South, north inbound = [1, 3]
#         west_east_signal_state = self.sim.roads[0].traffic_signal_state
#         west_east_n_vehicles = sum(len(self.sim.roads[i].vehicles) for i in [0, 2])
#         south_north_n_vehicles = sum(len(self.sim.roads[i].vehicles) for i in [1, 3])
#         out_bound_vehicles = sum(len(self.sim.roads[i].vehicles) for i in [4, 5, 6, 7])
#         # non_empty_junction = len(self.sim.intersections) != 0  # todo: validate,
